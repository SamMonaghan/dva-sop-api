...
class Seeds {
    ...
    public static void addSops(Repository repository) {
        try {
            ...
            ImmutableSet<InstrumentChange> newInstruments = Arrays.stream(registerIdsOfInitialSops).map(id -> new NewSop(id, OffsetDateTime.now()))
    ...
    public static void addServiceDeterminations(Repository repository)
    {
        try {
            ...
            ImmutableSet<InstrumentChange> newInstruments = Arrays.stream(registerIdsOfInitialServiceDeterminations).map(id -> new NewSop(id, OffsetDateTime.now()))
                    .collect(Collectors.collectingAndThen(Collectors.toList(), ImmutableSet::copyOf));
